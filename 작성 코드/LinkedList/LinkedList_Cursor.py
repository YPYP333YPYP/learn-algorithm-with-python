...
class ArrayLinkedList:
    ...
    def __init__(self, capacity: int):
        ...
        """ 해설 : 연결 리스트에서 노드를 삽입하고 삭제할 때마다 인스턴스 노드를 생성,소멸 한다면 비용이 상당할 것 이다.
            이를 해결하기 위해 '커서(cursor)'를 도입하는데 쉽게 말하면 배열의 인덱스 번호 이다. 
            또한 노드의 삭제 측면에서 '프리 리스트' 라는 개념이 도입됬는데 이는 삭제된 레코드 그룹을 관리할 때 사용하는 자료구조다.
            연결 리스트에서 노드가 삭제되면 프리 리스트로 옮겨져 관리 된다 """
